...
**Verzió:** 1.19 · **Állapot:** ÉLES, aktív fejlesztés (a Lapról Lapra modul v1) · **Projekt:** Collector app
...
### 5.1 Sorozat
UUID · kód · **kiadó** · **kategória (témakör)** (v1.16 — listatár-alapú, opcionális; lásd 10.2) · **megnevezés** (teljes név) · **megjelenítendő név** (fülön, max **16** karakter) · **szín** (bővíthető, családokba rendezett paletta, de bármennyi sorozat) · **komponens-készlet** (mely komponensekből áll egy szám ebben a sorozatban — a modul készletéből választva) · **borítókép** (v1.17 — lásd lent). **[DÖNTVE]**

**Sorozat-szintű borítókép [DÖNTVE, megvalósítva — v1.17]:** sok részletsorozatnál a számozott 1-es szám ELŐTT jár egy ingyenes bemutató-/reklámfüzet — ennek NINCS saját Száma (nem "0. szám"), csak egy tisztán vizuális/referencia célú kép-hely a sorozaton magán (`series.borito_url`, `series.borito_upload_enabled`). **Sehol nem számít bele** semmilyen darabszám/teljesítettségi-százalék számításba (`stats()` egyáltalán nem hivatkozik rá). Megjelenés: kicsi, álló formátumú (76×106px, `object-fit: contain`) miniatűr a Hero-doboz fejlécében, a kiadó/cím szövege mellett, bal oldalon — nem teljes szélességű banner; üresen szaggatott keretű placeholder. A workflow **szó szerint ugyanaz a mechanizmus**, mint a komponens-képeknél (5.3/2.7) — nincs párhuzamos kód: az `image_proposals` tábla `entity_type` ('component'/'series') megkülönböztetéssel bővült (`component_id` nullable, új `series_id` oszlop, CHECK a kizárólagosságra), a `js/component-images.js` függvényei egy `type` paraméterrel általánosítva (`uploadLiveImage`/`deleteLiveImage`/`setUploadEnabled`/`proposeImage`/`approveProposal`/`rejectProposal`), a megjelenítő `imageControlsHtml()` is közös. Jogosultság: a `can_propose_series_image()` SQL-függvény ugyanazt ellenőrzi, mint a komponenseknél a `can_propose_image()` — csak az javasolhat, aki a sorozatot már kiválasztotta a 📚 Sorozataimban. Draft/publikálás-ciklustól **független** (a komponens-képekhez hasonlóan), a `draft_series`-en nincs is mezője. SQL: `laprol-lapra-sorozat-borito.sql`.
...
- **Megjelenítés levágás nélkül [DÖNTVE, megvalósítva — v1.12]:** a lenyíló panelben (és a képjavaslat-elbírálás előnézetében) a kép `object-fit: contain`-nel jelenik meg mindenhol — a teljes kép mindig látszik, arányosan kicsinyítve, sosem vágva. Magazin/Könyv típusú komponenseknél a doboz alapból magasabb, keskenyebb (150×210px, kb. A4/1:1.4 arány), mert ezek szinte mindig álló (portré) borítók — a többi típusnál (Modell, Figura, Lego, Egyéb) a doboz mérete/aránya változatlan.
...
- **Kép törlése [DÖNTVE, megvalósítva — v1.13]:** staff (admin/owner) egy "🗑 Kép törlése" gombbal el is távolíthatja az élő képet, visszaállítva a komponenst "Nincs kép" állapotba — nem kell hozzá új kép feltöltése. Egyszerű, azonnali megerősítő kérdés védi (nincs türelmi idő, mert kis kockázatú: nem érint felhasználói adatot, nem kaszkádol semmit). Törléskor a Storage-fájl is törlődik (nem marad árva). "Nincs kép" állapotban a userek felé a feltöltés/javaslás automatikusan újra engedélyezett — ez a meglévő „ha nincs kép, mindig engedélyezett" szabályból következik, nem kellett külön mezőt módosítani hozzá.
...
2. **12 fölötti sorozatszám kezelése — 8–15 sorozatra tervezve** **[DÖNTVE, megvalósítva]**
   - **Színek:** bővített, **színcsaládokba csoportosított** paletta (6 család). A színválasztó vizuálisan csoportosítva mutatja a családokat.
   - **Kategória (témakör) — valódi mező, színcsalád-tematizálással [DÖNTVE, megvalósítva — v1.16, a v1.15-ös első próbálkozás után finomítva]:** a `series`/`draft_series` táblákon egy önálló, **listatár-alapú `kategoria` mező** (ugyanaz a minta, mint kiadó/komponens/azonosító/forrás — bővíthető a „☰ Listák” felületen, választható az Új sorozat/Karbantartás formon). A fülsáv-választó (`renderTabs`) **ténylegesen csoportosítva** jeleníti meg a sorozatokat témakör szerint (a listatár sort_order-je szerinti sorrendben, a kategória nélküliek egy "Egyéb" csoportban a végén). Kezdő témakörök: **Modellek → Kék** (II VH Repülők, Forma 1, Volkswagen, Fast & Furious), **Mese → Vörös** (Disney könyvek, Disney Hangoskönyvek, Mancs Őrjárat — a v1.15-ben próbált Magenta helyett, mert az túl hasonlónak/fakónak bizonyult a Kék/Zöld mellett), **Lego → Zöld** (Lego Star Wars, Lego Marvel — a család 8 árnyalatra bővítve, mert ez a legdinamikusabban bővülő kategória; 6 árnyalat tartalék a tervezett Spider-Man/Batman/Jurassic Park/Ninjago sorozatoknak). A Kék/Zöld családok hue/szaturációja is **élénkebbre igazítva** a v1.15-ös verzióhoz képest (magasabb szaturáció, a Zöld valódi zöld hue-ra tolva a korábbi teal-es árnyalatról) — az első verzió túl fakó/hasonló volt élesben. A **szín továbbra is önálló mező** marad (nincs automatikus kategória→szín-levezetés) — staff kézzel választ a témakörhöz illő családból, konvenció alapján. SQL: `laprol-lapra-kategoria-mezo.sql`.
...
5. Új tétel/komponens (nincs élő párja) új UUID-vel, `version=1`, diff/change_log nélkül kerül be (senkinek nincs még adata rajta) — **[hibajavítva, v1.10]** lásd lent, ez korábban örökre "eltüntethetetlen" felkiáltójelet okozott.
...
**Hibajavítás — az "eltüntethetetlen felkiáltójel" (v1.10):** a fenti 5. pont ("új tétel diff/change_log nélkül") egy komoly rést hagyott: egy vadonatúj Szám/komponens `version=1`-gyel jön létre, de SENKINEK nincs hozzá `member_seen` alapvonala (a hiányzó sor `0`-t ad, `1 > 0` örökre igaz) — miközben a `change_log`-ban nincs mit mutatni (semmi nem *módosult*, csak *létrejött*). Ez azt eredményezte, hogy a badge (verziószám-összehasonlítás) örökké aktív maradt, miközben az "Összes változás" modal (change_log-alapú) joggal üresnek látta — és mivel a "Mind elfogadom" is csak a KATTINTÓ saját `member_seen`-jét frissíti, a sorozatra feliratkozott TÖBBI user számára a jelzés örökre megmaradt. **Javítás:** minden ponton, ahol vadonatúj Szám/komponens létrejön (`publish_draft_series()`, a gyors "+ Új tétel" felvitel, Excel új sor), azonnal feltöltődik a `member_seen` alapvonala **mindenkinek**, aki jelenleg kiválasztotta a sorozatot (`seed_issue_seen_for_subscribers()` új SQL-függvény) — így a badge és a modal soha nem szakadhat el egymástól: jelzés csak ott keletkezhet, ahol tényleg van megjeleníthető change_log-bejegyzés is. Egyszeri visszatöltés is lefutott a már létrejött, alapvonal nélküli tételekre (csak azokra, amiknek SOHA nem volt change_log-bejegyzésük — a valódi, el nem fogadott változásokat ez nem érintette). SQL: `laprol-lapra-uj-tetel-seen-baseline.sql`.
...
**Hibajavítás — a lapszám nem volt újra felhasználható soft-delete után [v1.14].** Az `issues_series_id_lapszam_key` sima `UNIQUE(series_id, lapszam)` volt, a soft-delete-et figyelmen kívül hagyva — egy törölt Szám lapszáma örökre foglalt maradt, publikáláskor `duplicate key`-hibával elszállt egy új, ugyanolyan lapszámú Szám felvétele. Javítás: **részleges (partial) unique index**, `WHERE NOT is_deleted` — egy soft-deleted Szám lapszáma mostantól szabadon újra felhasználható. **Külön, kapcsolódó UX-csapda, amit érdemes elkerülni:** a draft-szerkesztő "Szám törlése a draftból" gombja (draft-items.js) CSAK a draft-tételt törli, az ÉLŐ Számot nem — ha valaki ezt használja "töröld a Számot" szándékkal, az élő adat érintetlen marad, és egy később hozzáadott, ugyanolyan lapszámú ÚJ tétel ütközni fog vele. A törzsadat tényleges törléséhez az adott (élő párral rendelkező) draft-tételen az "Ezt a Számot törlöm a sorozatból" jelölőnégyzet kell.

**Hibajavítás — lapszám-CSERE ütközött az egyediségi szabállyal [v1.18].** Két AKTÍV Szám sorszámának felcserélése (pl. #29↔#30) egy draftban publikáláskor ugyanazt a `duplicate key`-hibát dobta, de MÁS okból, mint a fenti v1.14-es eset: a `publish_draft_series()` a draft-tételeket SORBAN, egyesével UPDATE-eli — a csere köztes állapotában (miután csak az egyik UPDATE lefutott) átmenetileg KÉT Szám állna ugyanazzal a lapszámmal, amit a sima (nem halasztott) egyediség-ellenőrzés azonnal elbukott. A `publish_draft_series()` egyetlen PL/pgSQL FÜGGVÉNY (nem PROCEDURE) — ilyen szerkezetileg nem tartalmazhat `COMMIT`-ot, tehát a törzse mindig a hívó EGY tranzakciójában fut; csak az egyediség-ellenőrzés tranzakció-végi halasztása hiányzott. Mivel a v1.14 óta `issues_series_id_lapszam_key` egy RÉSZLEGES `UNIQUE INDEX` (ami sosem lehet `DEFERRABLE`), a javítás egy **`EXCLUDE USING btree (series_id WITH =, lapszam WITH =) WHERE (NOT is_deleted) DEFERRABLE INITIALLY DEFERRED`** constraint — ez Postgres-ben az egyetlen forma, ami EGYSZERRE támogatja a részleges feltételt ÉS a halasztást, tehát a v1.14-es és a v1.18-as javítás nem zárja ki egymást, egyetlen constraint fedi le mindkettőt. SQL: `laprol-lapra-lapszam-deferrable.sql`.
...
### 13.10 Változásnapló ("Újdonságok") **[DÖNTVE, megvalósítva — v1.15]**
🔔 fejléc-gomb, mindenkinek elérhető — egyszerű, lapozás nélküli lista a mindenkit érintő, LÁTHATÓAN megjelenő változásokról (nem minden apró hibajavítás, csak amit a user ténylegesen észrevesz a felületen), legújabb elöl. A tartalom a `js/changelog-content.js`-ben él (szándékosan elválasztva a megjelenítéstől, `js/changelog.js`), ugyanaz a szerkesztési minta, mint a súgónál. **Állandó szabály, mostantól minden érdemi, felhasználó által észlelhető változtatáshoz kötelező:** ha egy változás láthatóan/érezhetően megjelenik a felületen (nem csak a háttérben javít valamit), ahhoz egyszerű nyelvű changelog-bejegyzés kell — a súgóba a témába vágó helyen egy rövid magyarázat is kerülhet, ha a jelenség önmagában is megkérdezhető ("miért van ez így").

---

*Napló:*
- v1.19 — **Excel-import a draftban frissíti a meglévő lapszámot** (13.8): az „Excel-import kihagyja a meglévőt" viselkedés csak az 🔧 Gyors szerkesztés ⬆ Excel útvonalán (`excel.js`) volt kívánt módon frissítő; a draft-szerkesztő ⬆ Sablon feltöltése (`draft-excel.js`) szerkesztési draftban mindent kihagyott, mert a Szerkesztés indítása az összes élő Számot bemásolja. Most ez is frissít (nem üres cellák), előnézettel. Két út: A = 🔧 ⬆ Excel (automatikus szerkesztési draft + státusz/fizetett ár is), B = draft-szerkesztő sablon (csak törzsadat).
- v1.18 — **SÜRGŐS hibajavítás: lapszám-CSERE ütközött az egyediségi szabállyal** (13.8): két AKTÍV Szám sorszámának felcserélése (#29↔#30) ugyanazt a `duplicate key`-hibát dobta, mint a v1.14-es eset, de más okból — a publikálás soros, egyesével-UPDATE logikája a csere köztes állapotában átmenetileg két Számot hagyott ugyanazzal a lapszámmal, amit a sima (nem halasztott) egyediség-ellenőrzés azonnal elbukott. Mivel a v1.14-es részleges unique index sosem lehetett volna deferrable, a végleges forma egy `EXCLUDE ... WHERE ... DEFERRABLE INITIALLY DEFERRED` constraint, ami egyszerre részleges ÉS halasztott — a két korábbi javítás nem zárja ki egymást. Élesben tesztelve a valódi "Forma 1" #29/#30 sorain (tranzakción belül szimulálva, majd visszaállítva). SQL: `laprol-lapra-lapszam-deferrable.sql`.
- v1.17 — **Sorozat-szintű borítókép** (5.1): az 1-es szám előtti ingyenes bemutató-/reklámfüzethez egy önálló, tisztán vizuális/referencia mező a sorozaton (nem Szám, sehol nem számít bele darabszám/százalék-számításba) — kicsi, álló formátumú miniatűr a Hero-doboz fejlécében, a cím/kiadó mellett. A workflow a MEGLÉVŐ komponens-kép mechanizmus általánosítása (`image_proposals.entity_type`, `js/component-images.js` type-paraméterezve) — nincs párhuzamos kód. SQL: `laprol-lapra-sorozat-borito.sql`.
- v1.16 — **Valódi Kategória (témakör) mező + élénkebb színek** (10.2, 2. pont) — finomítás a v1.15-ös első próbálkozáson, élesben kapott visszajelzés alapján ("nagyon hasonló színek lettek", "jobb lenne élénkebb"). Önálló, listatár-alapú `kategoria` mező a sorozatokon (nem csak a szín családjából "visszafejtve") — a fülsáv-választó ténylegesen csoportosít témakör szerint. A Mese témakör a Magenta helyett a MEGLÉVŐ, eleve élénk Vörös családot kapja; a Kék/Zöld családok szaturációja jelentősen megemelve, a Zöld hue-ja teal-ről valódi zöldre tolva. SQL: `laprol-lapra-kategoria-mezo.sql`. Súgó + changelog frissítve (a v1.15-ös bejegyzés felülírva a végleges állapottal, mert az köztes, rövid életű verziót írt le).
- v1.15 — **Sorozat-színek tematizálása kategóriánként** (10.2, 2. pont): Modellek → Kék, Mese → Magenta, Lego → Zöld családok, minden család a meglévő hue/szaturációt megtartva, csak a világosság-lépcső bővítve szisztematikusan (Kék 4→5, Magenta 4→4 újragenerálva, Zöld 4→8 árnyalat). A 9 jelenleg létező érintett sorozat átszínezve. SQL: `laprol-lapra-sorozat-szin-tematizalas.sql`. **Új, állandó rendszer: Változásnapló ("Újdonságok")** (13.10) — 🔔 fejléc-gomb, `js/changelog-content.js`/`changelog.js`; mostantól minden láthatóan megjelenő, felhasználó által észlelhető változtatáshoz kötelező egy egyszerű nyelvű bejegyzés ide. Első bejegyzés: a sorozat-szín-tematizálásról. A súgó is frissült (a fejléc-gombok listája + egy új Q&A a fülszín-témáról).
- v1.14 — **SÜRGŐS hibajavítás: publikálás ütközött soft-delete-elt/élő lapszámmal** (13.8): az `issues_series_id_lapszam_key` sima `UNIQUE(series_id, lapszam)` volt, a soft-delete-et figyelmen kívül hagyva — egy törölt Szám lapszáma örökre foglalt maradt. Konkrét eset: a "Volkswagen modellautógyűjtemény" draftjában 20 db, hibás AI-adatot javító, élő pár nélkülinek maradt tétel (#4-19, #27, #30, #59, #61 — a "Szám törlése a draftból" gomb véletlen használata miatt szakadtak el az élő párjuktól, ami csak a draft-tételt törli, az élőt nem) vadonatúj beszúrásként ütközött a még élő, régi sorral. Javítás: **részleges unique index** (`WHERE NOT is_deleted`) — soft-delete után a lapszám újra felhasználható; a konkrét draft 20 tétele lapszám (és típus) alapján visszakötve az élő párjához, adatvesztés nélkül, hogy szerkesztésként (ne ütköző beszúrásként) publikálódjanak. SQL: `laprol-lapra-lapszam-reszleges-unique.sql`.
- v1.13 — **Kép törlésének lehetősége** (5.3): staff (admin/owner) "🗑 Kép törlése" gombbal eltávolíthatja az élő komponens-képet (visszaáll "Nincs kép" állapotra), egyszerű azonnali megerősítéssel — nincs türelmi idő, kis kockázatú művelet. A Storage-fájl is törlődik. Nincs SQL-változás: a meglévő staff-jogosultság (`is_staff()`) és a Storage staff-policy (`for all`) már eddig is fedte ezt; a "Nincs kép → userek felé mindig engedélyezett a feltöltés" szabály is a meglévő `!kep_url` feltételből automatikusan érvényesül.
- v1.12 — **Komponens-kép levágás nélküli megjelenítése** (5.3): a hibajavítási kör 15. pontja ("nagyobb kép-placeholder") kiegészítve — a nagyobb doboz önmagában nem oldotta meg, hogy magas, álló formátumú magazin-borítóknál (pl. Forma-1 #28, II. VH Repülők #35) a `cover`-vágás levágta a cím/kiadói logó sávot. Váltás `object-fit: contain`-re mindenhol, ahol komponens-kép megjelenik (panel + képjavaslat-előnézet); Magazin/Könyv típusnál a doboz 150×210px-re (kb. A4-arány) állítva, a többi típusnál változatlan.
- v1.11 — **A 1000-soros lapozási hiba teljeskörű átvizsgálása és javítása.** A v1.10-es hibajavítás után átnéztük a TELJES kódbázist: minden `.select()` hívást, aminek nincs explicit lapozása, és aminek a sorszáma elvben átléphetné a Supabase/PostgREST alapértelmezett 1000-es limitjét. A közös `fetchAllRows()` segéd (`js/supabase.js`) mostantól ott is lapoz, ahol korábban nem: `change_log`/`member_seen` egy sorozat összes tételére (`js/changes.js` — a change_log sosem törlődik, entitásonként több sort is felhalmozhat), `components` egy nagy sorozat/draft összes tételére (`js/karbantartas.js`, `js/my-series.js`, `js/draft-items.js` — a "több azonos típusú komponens" funkció óta ez nagyobb lehet, mint korábban), és két app-szintű, MINDEN userre vonatkozó lekérdezés (`member_series`, `members` — `js/karbantartas.js`, `js/admin-users.js`), amik (userek × sorozatok) illetve a regisztrált userek számával nőnek.
- v1.10 — **Sürgős hibajavítás: az "eltüntethetetlen felkiáltójel"** (13.6): vadonatúj Szám/komponens `version=1`-gyel, `change_log`-bejegyzés és `member_seen`-alapvonal nélkül jött létre — a badge (verziószám-alapú) örökké aktív maradt, a "Összes változás" modal (change_log-alapú) meg joggal üresnek látta, és a "Mind elfogadom" is csak a kattintó saját alapvonalát javította, a sorozatra feliratkozott többi usernek nem. Élesben megerősítve a "Fast & Furious modellek" #1 tételén: a tulajdonosnak már rendben volt (korábbi "Mind elfogadom" működött), de egy másik feliratkozott usernek egyetlen `member_seen` sora sem volt rá. Javítás: `seed_issue_seen_for_subscribers()` új SQL-függvény minden ponton lefut, ahol új Szám/komponens létrejön (`publish_draft_series()`, "+ Új tétel", Excel új sor) — MINDEN jelenlegi feliratkozónak azonnal alapvonalat ad, így badge soha nem keletkezhet megjeleníthető change_log nélkül. Egyszeri, biztonságos visszatöltés a már létrejött, alapvonal nélküli (és SOHA nem módosult) tételekre. SQL: `laprol-lapra-uj-tetel-seen-baseline.sql`. **Melléktalálat:** a `db.local.js` migrációs kapcsolat "Direct connection"-ról "Session pooler"-re váltva (8. fejezet) — a Direct connection host kizárólag IPv6-címet publikál, ami egy munkamenetből elérhetetlennek bizonyult; a Session pooler IPv4-kompatibilis és bizonyítottan jó tranzakciós migrációkhoz is.
